...
class FakeCrawler[F[_]] extends Crawler[F] {
  override def fetchTitles(urls: List[Refined[String, string.Uri]]): F[List[Crawler.TitleFetchingResult]] = {
    ...
    cats.effect.IO.pure(results).asInstanceOf[F[List[Crawler.TitleFetchingResult]]]
  }
}
